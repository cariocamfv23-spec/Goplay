import { An as Trophy, Ar as Calendar, Cn as Zap, Hn as ShieldCheck, Ir as ArrowLeft, Jn as MessageSquare, Mn as TrendingDown, Nr as Bell, Pn as Target, Pr as Baby, Sn as cn, Sr as Clock, V as Badge, Xn as Megaphone, Yn as MessageCircle, _r as Eye, cr as Handshake, ei as require_jsx_runtime, in as AvatarImage, jn as TrendingUp, nn as Avatar, or as History, rn as AvatarFallback, si as useNavigate, sn as Button, sr as Heart, ur as GraduationCap, xr as CloudLightning, yr as Crown, z as useNotificationStore_default } from "./index-CVX6HDj3.js";
...
function Notifications() {
	...
	const getIcon = (type, title = "", message = "") => {
		switch (type) {
			...
			case "memory": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(History, { className: "h-5 w-5 text-purple-500" });
	...
	const getPriorityStyle = (priority, title = "", message = "", type) => {
		if (type === "memory") return "border-l-4 border-l-purple-500 bg-purple-500/10 dark:bg-purple-900/20";
			...
			children: hasNotifications ? Object.entries(groupedNotifications).map(([date, items]) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h3", {
					...
					children: items.map((not) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: cn("p-4 rounded-xl flex gap-4 transition-all relative overflow-hidden border border-border/30 shadow-sm", not.read ? "bg-card opacity-90" : "bg-card ring-1 ring-primary/20 shadow-md", getPriorityStyle(not.priority, not.title, not.message, not.type), not.link && "cursor-pointer active:scale-[0.98]"),
						...
						children: [
							not.user ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								...
								className: cn("mt-1 h-10 w-10 rounded-full flex items-center justify-center shrink-0 border border-border/50", not.read ? "bg-secondary/50" : "bg-secondary", isVipAlert(not.title, not.message) && "bg-gold/10 border-gold/30", not.title.toLowerCase().includes("scout") && "bg-blue-500/10 border-blue-500/30", not.title.toLowerCase().includes("sponsor") && "bg-emerald-500/10 border-emerald-500/30", not.type === "system_update" && "bg-primary/10 border-primary/30", not.type === "memory" && "bg-purple-500/10 border-purple-500/30"),
...
//# sourceMappingURL=Notifications-GGA-rWMc.js.map