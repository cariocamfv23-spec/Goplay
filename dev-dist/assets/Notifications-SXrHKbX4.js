import { B as useFlashbackStore, Cr as CloudLightning, Fr as Bell, In as Target, Ir as Baby, Mn as Trophy, Mr as Calendar, N as usePrivacyStore, Nn as TrendingUp, Pn as TrendingDown, Qn as Megaphone, Rr as ArrowLeft, Sn as toast, Tn as Zap, U as Badge, V as useNotificationStore_default, Wn as ShieldCheck, Xn as MessageSquare, Zn as MessageCircle, an as AvatarFallback, cr as History, fr as GraduationCap, in as Avatar, li as useNavigate, ln as Button, lr as Heart, ni as require_jsx_runtime, on as AvatarImage, ur as Handshake, wn as cn, wr as Clock, xr as Crown, yr as Eye } from "./index-yBhgE2__.js";
var import_jsx_runtime = require_jsx_runtime();
function Notifications() {
	const navigate = useNavigate();
	const { isPreviewLocked } = usePrivacyStore();
	const { notifications, markAsRead, markAllAsRead, unreadCount, addNotification } = useNotificationStore_default();
	const { openFlashback } = useFlashbackStore();
	const isVipAlert = (title, message = "") => {
		const text = (title + " " + message).toLowerCase();
		return text.includes("vip") || text.includes("scout") || text.includes("sponsor");
	};
	const getIcon = (type, title = "", message = "") => {
		switch (type) {
			...
			case "verification":
				if (title.toLowerCase().includes("scout")) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Eye, { className: "h-5 w-5 text-blue-500" });
				if (title.toLowerCase().includes("sponsor")) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(Handshake, { className: "h-5 w-5 text-emerald-500" });
	...
	const getPriorityStyle = (priority, title = "", message = "", type) => {
		...
		if (isVipAlert(title, message)) {
			if (title.toLowerCase().includes("scout")) return "border-l-4 border-l-blue-500 bg-blue-500/10 dark:bg-blue-900/20";
			if (title.toLowerCase().includes("sponsor")) return "border-l-4 border-l-emerald-500 bg-emerald-500/10 dark:bg-emerald-900/20";
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [!isPreviewLocked && /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Button, {
					variant: "outline",
					size: "sm",
					...
					onClick: () => {
						...
						toast.custom((t) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "flex w-full items-center gap-3 rounded-2xl border-2 border-purple-500/30 bg-background/95 p-3 shadow-2xl backdrop-blur-xl animate-in slide-in-from-top-4 duration-300",
							children: [
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
									className: "flex h-12 w-12 shrink-0 items-center justify-center rounded-xl bg-purple-500/20 text-purple-500",
								...
								/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
									...
									children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
										...
										children: "Lembrança Desbloqueada!"
									}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
										className: "text-xs text-muted-foreground line-clamp-2 leading-tight",
										children: "Você tem um Flashback esperando por você."
									})]
								}),
								/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
									onClick: () => {
										toast.dismiss(t);
										useNotificationStore_default.getState().markAsRead(nid);
										openFlashback("today");
									},
					...
					children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)(History, { className: "w-3 h-3 mr-1" }), "Simular Memória"]
			...
			children: hasNotifications ? Object.entries(groupedNotifications).map(([date, items]) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h3", {
					...
					children: items.map((not) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: cn("p-4 rounded-xl flex gap-4 transition-all relative overflow-hidden border border-border/30 shadow-sm", not.read ? "bg-card opacity-90" : "bg-card ring-1 ring-primary/20 shadow-md", getPriorityStyle(not.priority, not.title, not.message, not.type), (not.link || not.type === "time_travel" || not.type === "memory") && "cursor-pointer active:scale-[0.98]"),
						onClick: () => handleNotificationClick(not),
						children: [
							not.user ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
								className: "relative shrink-0 mt-1",
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsxs)(Avatar, {
										className: cn("h-10 w-10 ring-2 ring-offset-2 ring-offset-background", not.type === "live_stream" ? not.link?.includes("/replay/") ? "ring-primary" : "ring-red-500" : "ring-transparent border border-border"),
								...
								className: cn("mt-1 h-10 w-10 rounded-full flex items-center justify-center shrink-0 border border-border/50", not.read ? "bg-secondary/50" : "bg-secondary", isVipAlert(not.title, not.message) && "bg-gold/10 border-gold/30", not.title.toLowerCase().includes("scout") && "bg-blue-500/10 border-blue-500/30", not.title.toLowerCase().includes("sponsor") && "bg-emerald-500/10 border-emerald-500/30", not.type === "system_update" && "bg-primary/10 border-primary/30", (not.type === "memory" || not.type === "time_travel") && "bg-purple-500/10 border-purple-500/30"),
								...
								children: [
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
										...
										children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("h4", {
											className: cn("font-bold text-sm leading-tight", not.read ? "text-foreground/80" : "text-foreground", not.priority === "critical" && "text-red-600 dark:text-red-400", isVipAlert(not.title, not.message) && "text-gold", not.title.toLowerCase().includes("scout") && "text-blue-600 dark:text-blue-400", not.title.toLowerCase().includes("sponsor") && "text-emerald-600 dark:text-emerald-400", not.type === "live_stream" && !not.link?.includes("/replay/") && "text-red-500", (not.type === "memory" || not.type === "time_travel") && "text-purple-600 dark:text-purple-400"),
											children: not.title
										})
									}),
									/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
...
//# sourceMappingURL=Notifications-SXrHKbX4.js.map