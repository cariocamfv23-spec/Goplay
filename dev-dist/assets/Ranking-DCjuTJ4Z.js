...
import { a as SelectValue, i as SelectTrigger, n as SelectContent, r as SelectItem, t as Select } from "./select-BmUGbdpR.js";
import { t as Gift } from "./gift-HzU1afRw.js";
import { t as Minus } from "./minus-BmKELCRC.js";
import { t as Shield } from "./shield-D092YFEW.js";
import { An as Trophy, Ct as mockRankings, Dr as ChevronRight, En as Users, Mn as TrendingDown, Nr as Calendar, Q as mockCurrentUser, Qn as Medal, R as Badge, Rn as Star, Sn as Zap, St as mockPsychologicalRankings, Y as mockChallenges, ar as Info, c as DialogTrigger, ci as useSearchParams, cn as Tooltip, dn as TooltipTrigger, hr as Flame, i as DialogDescription, jn as TrendingUp, ln as TooltipContent, mi as require_react, nn as AvatarFallback, o as DialogHeader, on as Button, r as DialogContent, ri as require_jsx_runtime, rn as AvatarImage, s as DialogTitle, t as Dialog, tn as Avatar, ui as useNavigate, wr as Clock, xn as cn, xr as Crown, zr as ArrowLeft } from "./index-C-w04ldD.js";
import { n as CardContent, t as Card } from "./card-CYMIDGTz.js";
import { t as Progress } from "./progress-BuxSjEXg.js";
import { i as TabsTrigger, r as TabsList, t as Tabs } from "./tabs-DfpQlF33.js";
import { t as ptBR } from "./pt-BR-B7zprBc_.js";
import { t as format } from "./format-Bywjss7D.js";
import { t as formatDistanceToNow } from "./formatDistanceToNow-CV-OOnE9.js";
var import_react = require_react();
var seededRandom = (seed) => {
	const x = Math.sin(seed) * 1e4;
	return x - Math.floor(x);
};
const getRankings = (timeRange, metric) => {
	const extraUsers = Array.from({ length: 20 }).map((_, i) => {
		const seed$1 = i + 1e3;
		const hasAchievement = seededRandom(seed$1) > .75;
		let achievement;
		if (hasAchievement) {
			const type = seededRandom(seed$1 + 1);
			if (type > .8) achievement = {
				type: "mvp",
				label: "MVP da Rodada"
			};
			else if (type > .5) achievement = {
				type: "streak",
				label: "Em Chamas"
			};
			else if (type > .25) achievement = {
				type: "veteran",
				label: "Veterano"
			};
			else achievement = {
		...
		return {
			id: `dummy_${i}`,
			position: 0,
			points: 1500 - i * 60 + Math.floor(seededRandom(seed$1 + 2) * 200),
			trend: seededRandom(seed$1 + 3) > .6 ? "up" : seededRandom(seed$1 + 3) > .3 ? "down" : "same",
			user: {
				id: `u_dummy_${i}`,
				name: `Atleta ${[
					"Alpha",
	...
	const timeFactor = Date.now() / 2e4;
	const fluctuation = Math.floor(Math.sin(timeFactor) * 300);
	...
	const processedData = allEntries.map((entry) => {
		if (entry.user.id === mockCurrentUser.id) return entry;
		...
		const randomVariance = entry.user.id.charCodeAt(entry.user.id.length - 1) * seed % 50 - 25;
...
const getChallengeRankings = (challengeId) => {
	return getRankings("all_time", mockChallenges.find((c) => c.id === challengeId)?.metric || "points");
};
...
//# sourceMappingURL=Ranking-DCjuTJ4Z.js.map