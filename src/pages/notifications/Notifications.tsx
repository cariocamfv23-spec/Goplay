...
import { useTheme } from 'next-themes' // Added for theme awareness if needed, though not strictly used
      ...
      <div className="p-4 space-y-2">
        {notifications.map((not) => (
          <div
            key={not.id}
            className={cn(
              'p-4 rounded-xl flex gap-4 transition-colors relative overflow-hidden',
              not.read ? 'bg-background' : 'bg-primary/5',
              getPriorityStyle(not.priority),
              not.link &&
                'cursor-pointer hover:bg-secondary/50 active:scale-95',
            )}
          ...
          >
            ...
            {!not.read && (
              <div className="h-2 w-2 rounded-full bg-primary mt-2 absolute top-4 right-4"></div>
            )}
          </div>
        ))}
